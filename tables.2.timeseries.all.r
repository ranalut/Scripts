...
for (i in 1:length(models2))
{
	...
	png(paste(workspace,'Analysis/pop.',scenario,'smz.',models2[i],'.png',sep=''),res=175, height=750, width=750)
	par(mar=c(4,4,1,1))
	
	temp <- the.data[the.data$model==models2[i] & the.data$gcm==theGCMs[1] & the.data$replicate==1,'Trait.Index..1']
	temp <- temp[-1]
	plot(temp[10:110] ~ seq(2000,2100,1), type='l', ylim=c(yMin,yMax), col=myColors[1], xlab='YEAR', ylab='FEMALES')
