...
##################################################
# Linear models (w/ bootMer SE) with a transformed response variable. 
...
# =====================================================
ly$ln_pop <- log(ly$pop)
hist(ly$ln_pop)
shapiro.test(ly$ln_pop) # significantly different from normal
# hist(ly$d2090s_dens)
# stop('cbw')
bc <- boxcoxfit(ly$pop, lambda2=TRUE)
ly$bc_pop <- (ly$pop)^(bc$lambda[1]) 
hist(ly$bc_pop, breaks=20)
...
print(AIC(ly_01)) # Poisson  33112.9 ******
...
print(AIC(ly_02)) # Poisson 33785.05
...
print(AIC(ly_03)) # Poisson 37107.29
...
print(AIC(ly_04)) # Poisson  35320.71
...
print(AIC(ly_05)) # Poisson 38642.95
...
print(AIC(ly_01a)) # Poisson 26182.92 ******
plot(ly_01a)
qqnorm(resid(ly_01a), main="Q-Q plot for residuals")
qqline(resid(ly_01a),col='red')
print(summary(ly_01a)) # Adjusted R^2 = 98.5%, Deviance explained 98.7%
...
print(AIC(ly_01b)) # Poisson 26326.77
