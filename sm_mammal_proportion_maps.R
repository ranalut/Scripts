...
study.area <- readOGR(dsn='D:/Box Sync/PNWCCVA/PNWCCVA_GeoDatabases/PNWCCVA_pygmy_rabbit.gdb',layer="pnwccva_us_study_area")
 
huc <- readOGR(dsn='D:/Box Sync/PNWCCVA/PNWCCVA_GeoDatabases/PNWCCVA_pygmy_rabbit.gdb',layer="pnwccva_watersheds_us")
shapefiles <- list(pol=political,sa=study.area,huc=huc)

################################################
# Inputs
species <- c('townsends_squirrel_complex','pygmy_rabbit','kangaroo_rat')# c('ground squirrel','pygmy rabbit','kangaroo rat')
gcms <- c('ccsm3','cgcm3','giss','hadcm3','miroc') # 'ave'
dbf_names <- c('_squirrel_11_hab_v2','_rabbit_020_hab_v2','_krat_05_hab_v2')
scenarios <- c('full','full','full')
years <- c('Y2000','Y2020','Y2050','Y2090')
...
plot.dens <- function(spp, gcm, scenario, dbf_name, breaks, year, spatial_data=shapefiles)
{
  ...
  ###############################################
  # Plot
  png(paste("D:/Box Sync/PNWCCVA/MS_SmallMammal/Plots/",spp,'_',gcm,'_',year,".png",sep=''),width=260,height=200)
  par(mar=c(0,0,0,0))
  plot(spatial_data[['pol']], col=rgb(189,189,189,max=255), xlim=c(-125,-102), ylim=c(38,50)) #, cex.axis=1.25)
