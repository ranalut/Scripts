...
# index <- 1
markers <- rep('-a',7) # rep('',7) # c(index,rep('',5),index) # rep('-b',7) 
scenario <- 'lulc.' # 'clim.' # '' # 'lulc.' # 'veg.'
...
for (n in c(seq(1,110,1),1))
{
  ...
  for (i in 2:length(map.names))
	...
	write.csv(data.frame(hexid=the.data$hexid,Pred=pred.spp.distn[,2]),paste('l:/space_lawler/shared/wilsey/postdoc/hexsim/workspaces/rabbit_v1/analysis/rf.model.pred',markers[1],'.csv',sep=''),row.names=FALSE)
	file.remove(paste(hexsim.wksp,'workspaces/',spp.folder,'/Analysis/',map.names[export.maps],markers[export.maps],'.csv',sep=''))
