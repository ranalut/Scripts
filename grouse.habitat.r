...
# index <- seq(1,5,1)
markers <- rep('',7) # rep('-a',7) # rep('',7) # c(index,rep('',5),index) # rep('-b',7) 
...
for (k in 1:5)
{
	...
	for (n in c(seq(1,110,1),1))
	{
		...
		if (scenario=='clim.') { time.steps['biomes'] <- 30; time.steps['lulc'] <- 32; export.maps <- c(2:5) } # Only climate changes.
		if (scenario=='lulc.') { time.steps['biomes'] <- 30; time.steps[c('def.mam','fire','mtco','mtwa')] <- 1; export.maps <- 7 } # Only lulc changes.
		if (scenario=='veg.') { time.steps['lulc'] <- 32; time.steps[c('def.mam','fire','mtco','mtwa')] <- 1; export.maps <- 6 } # Only veg changes.
		print(time.steps)# ; stop('cbw')
		if (n==1)
		{
		for (i in 1:length(map.names))
		{
			export.hexmaps.spatial(hexsim.wksp2=hexsim.wksp2, spp.folder=data.folder, hexmap.name=map.names[i],time.step=time.steps[i],marker=markers[i]) # stop('cbw')
		}
		}
		else
		{
		...
		# Assemble data
		the.data <- read.csv(paste(hexsim.wksp,'workspaces/',data.folder,'/Analysis/',map.names[1],markers[1],'.csv',sep=''))
		cat(col.names[2],' ')
		
		for (i in 2:length(map.names))
		{
			temp <- read.csv(paste(hexsim.wksp,'workspaces/',data.folder,'/Analysis/',map.names[i],markers[i],'.csv',sep=''),row.names=1)
			the.data <- cbind(the.data,temp)
			cat(col.names[i+1],' ')
		}
		
		colnames(the.data) <- col.names
		the.data <- the.data[the.data$water!=1 & the.data$def.mam!=0 & the.data$lulc!=0,]
		the.data <- the.data[-1,]
		...
		# Load Model
		load(file=paste('l:/space_lawler/shared/wilsey/postdoc/hexsim/workspaces/',spp.folder,'/analysis/rf.model.v1.rdata',sep=''))
		
		pred.spp.distn <- predict(rf.model, newdata=the.data, type='prob')

		write.csv(data.frame(hexid=the.data$hexid,Pred=pred.spp.distn[,2]),paste('l:/space_lawler/shared/wilsey/postdoc/hexsim/workspaces/',spp.folder,'/analysis/rf.model.pred',markers[1],'.csv',sep=''),row.names=FALSE)
		file.remove(paste(hexsim.wksp,'workspaces/',data.folder,'/Analysis/',map.names[export.maps],markers[export.maps],'.csv',sep=''))
		cat('completed prediction\n')
		
		hexmap.name <- paste('hab.',scenario,theGCM,sep='')
		cat(hexmap.name,'\n')
		import.hexmaps(hexsim.wksp2=hexsim.wksp2, spp.folder=spp.folder, csv.name=paste('rf.model.pred',markers[i],sep=''), hexmap.name=paste('temp.', hexmap.name, sep=''))
		...
		file.copy(
			...
			to=paste(hexsim.wksp,'Workspaces/',spp.folder,'/Spatial Data/Hexagons/',hexmap.name,'/',hexmap.name,'.',n,'.hxn',sep=''),overwrite=TRUE
			)
		cat('hexmap imported\n')
		
		file.remove(paste(hexsim.wksp,'Workspaces/',spp.folder,'/Spatial Data/Hexagons/temp.',hexmap.name,'/temp.',hexmap.name,'.1.hxn',sep=''))
		# unlink(paste(hexsim.wksp,'Workspaces/',data.folder,'/Spatial Data/Hexagons/temp.',hexmap.name,'/',sep=''))
