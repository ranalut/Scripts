...
output2 <- read.csv("D:/Box Sync/PNWCCVA/MS_MesoCarnivores/Results/yr_pop_table_thresh.csv",header=TRUE,stringsAsFactors=TRUE,row.names = 1)

output2$sens <- factor(output2$sens)
output2$gcm <- factor(output2$gcm)
# output2$ECO_CODE <- factor(output2$ECO_CODE)
output2$dens <- 100 * output2$pop/output2$AREA_SQKM
...
hist(ly$dens) # Not normally distributed.
hist(ly$dens^(1/3))
# stop('cbw')

# m_ly1 <- lmer(d2050s ~ sens + gcm + (1 | ECO_CODE), data=ly, REML=FALSE) 
# m_ly1_n <- lmer(d2050s ~ gcm + (1 | ECO_CODE), data=ly, REML=FALSE) 
m_ly2 <- lmer(dens ~ sens + gcm + year + (1 + sens|ECO_CODE), data=ly, REML=FALSE)
m_ly2_n <- lmer(dens ~ gcm + year + (1 + sens|ECO_CODE), data=ly, REML=FALSE)
