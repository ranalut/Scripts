...
post.processing.maps <- function(workspace,scenario,models2,theGCMs,reps,spatial.name,ref.field,ref.values,out.name)
{
	...
	for (i in 1:length(theGCMs))
	{
		for (j in 1:length(models2))
		{
			...
			temp <- round(temp)
			# print(head(temp)); print(tail(temp)); stop('cbw')
