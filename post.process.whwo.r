...
# for (map in 1:3) { source('post.process.whwo.r') }

source('export.hexmaps.r')

spp.folder <- 'woodpecker_v1' # 'town_squirrel_v1' # 'krat_v1' # 'rabbit_v1' # 'woodpecker_v1' # 'nutcracker_v1' 
map.name <- 'distribution'
scenario <- 'whwo.14.hab.v2.' # 'squirrel.11.hab.v2.' # 'krat.05.hab.v2.' # 'rabbit.020.hab.v2.'
theGCMs <- c('CCSM3','CGCM3.1_t47','GISS-ER','MIROC3.2_medres','UKMO-HadCM3')
# theGCMs <- c('ccsm3','cgcm3','giss-er','miroc','hadcm3')
model.types <- list(c('','clim.'),c('full','clim'))
reps <- 25
...
scenarios <- rep(scenario,(length(model.types[[1]])*2*5*25*11))
model_1 <- rep(model.types[[1]],each=(5*25*11))
model_2 <- rep(model.types[[2]],each=(5*25*11))
gcm <- rep(rep(theGCMs,each=(25*11)),2)
rep <- rep(rep(seq(1,reps,1), each=11),(2*5))
year <- rep(time.steps,(2*5*25))
