...
huc <- read.dbf('D:/Box Sync/PNWCCVA/Outputs/pnwccva_watersheds_us.dbf',as.is=TRUE)

species <- c('ground squirrel','pygmy rabbit','kangaroo rat')
scenario <- c('_squirrel_11_hab_v2','_rabbit_020_hab_v2','_krat_05_hab_v2')
sens <- list(c('full','veg','clim'), c('full','veg','clim'),c('full','veg','clim'))
path <- c('townsends_squirrel_complex/townsends_squirrel_complex_huc_','pygmy_rabbit/pygmy_rabbit_huc_','kangaroo_rat/kangaroo_rat_huc_') 
dens_thresh <- c(0.01,0.01,0.01)
...
#############################################
# Area
output4 <- gather(output3, key=year, value=area, -PNWCCVA_ID, -AREA_SQKM_, -species, -sens, -gcm)
# spp <- species
# output5 <- list()
...
bar_table <- aggregate(area ~ species + sens + gcm + year ,data=output4, FUN=sum)
# print(bar_table); stop('cbw')

bar_table2 <- filter(bar_table, (sens=='50' | sens=='full')) # & species=='lynx'

temp <- filter(bar_table2, year=='Y2000')
temp <- rbind(temp,temp,temp)
bar_table3 <- filter(bar_table2, year!='Y2000')
bar_table3$year <- substr(bar_table3$year,2,5)
...
png("D:/Box Sync/PNWCCVA/MS_SmallMammal/Results/pd_acreages3.png",height=500)
  plot(p)
dev.off()
write.csv(bar_table3,"D:/Box Sync/PNWCCVA/MS_SmallMammal/Results/delta_acreages3.csv")
stop('cbw')
