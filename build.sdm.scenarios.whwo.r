...
theScenarios <- c('','clim.')
base.name <- 'whwo.15.' # 'squirrel.11.' # 'krat.05.' # 'rabbit.020.'
thresholds <- as.character(c(0.52,0.525)) # as.character(c(0.56,0.51,0.838)) # as.character(c(0.55,0.495,0.772)) # as.character(c(0.63,0.55,0.656))
workspace <- 'woodpecker_v1' # 'town_squirrel_v1' # 'krat_v1' # 'rabbit_v1'
...
for (i in theGCMs)
{
	for (j in 1:3)
	{
		...
		temp <- gsub(pattern='0.52',replacement=thresholds[j],x=temp, ignore.case=TRUE)
