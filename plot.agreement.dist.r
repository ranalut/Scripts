...
create.figure.agreement <- function(workspace, species.folder, baseline.scenario, future.scenario, spatial.variable, baseline.time, time.window, cutoffs, data.type, min.occ)
{
	# ================================================================================================
	# Spatial Data
	...
	for (i in 1:dim(fut.data[[1]])[1])
	{
		...
		{
			the.value <- length(the.data[the.data > min.occ])
	...
	png(paste(workspace,species.folder,'/analysis/dist.agree.',future.scenario[1],data.type,'.',time.window[1],future.scenario[2],'.png',sep=''), width=450, height=400)
...
create.figure.agreement(
	...
	baseline.time='31.40',
	time.window=c('99.109','LATE-2000s'), # c('51.60','MID-2000s'), c('99.109','LATE-2000s'),
