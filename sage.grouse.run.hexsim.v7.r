...
theRanges.orig <- read.table(paste(workspace,'Results/sagr.',simNumber,'/sagr.',simNumber,'_ranges_sage_grouse.csv',sep=''),sep=',',fill=TRUE,skip=1,stringsAsFactors=FALSE,col.names=paste('V',seq(1,1000,1),sep=''))
...
theCensus <- read.csv(paste(workspace,'Results/sagr.',simNumber,'/sagr.',simNumber,'.0.csv',sep=''),header=TRUE)
print('Census Numbers')
print(theCensus[,2:6])
cat('scenario number',simNumber,'\n')

theCensus2 <- read.csv(paste(workspace,'Results/sagr.',simNumber,'/sagr.',simNumber,'.1.csv',sep=''),header=TRUE)
theCensus2 <- theCensus2[,(6+seq(4,16,2))]
