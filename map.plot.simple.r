...
map.plot.simple <- function(the.data, distribution.data, cutoffs, spatial.data, political, ocean, model.name, add.legend, include.axes, color.ramp, margins, label.text=NA, study.area, data.type)
{
	the.map <- merge(spatial.data,the.data,all.x=FALSE)
	the.range1 <- range(the.map@data$variable); print(the.range1); print(length(the.map@data$variable))
	...
	if (add.legend==TRUE) 
	{
		image.plot(legend.only=TRUE, add=TRUE, zlim=c(min(cutoffs),max(cutoffs)), breaks=cutoffs, col=color.ramp, lab.breaks=cutoffs, legend.width=3, graphics.reset=TRUE, legend.args=list(text=label.text,cex=1.5,side=1,line=ifelse(data.type=='abundance',2,2.5)))
	}
}	
