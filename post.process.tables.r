...
post.processing <- function(workspace,scenario,models2,theGCMs,reps,spatial.name,ref.field,ref.values,out.name)
{
	...
	for (i in 1:length(theGCMs))
	{
		for (j in 1:length(models2))
		{
			...
			temp$Y2000s <- round(apply(temp[,c(paste('Y',seq(2000,2009,1),sep=''))],1,mean))
			temp$Y2020s <- round(apply(temp[,c(paste('Y',seq(2020,2029,1),sep=''))],1,mean))
			temp$Y2050s <- round(apply(temp[,c(paste('Y',seq(2050,2059,1),sep=''))],1,mean))
			temp$Y2090s <- round(apply(temp[,c(paste('Y',seq(2090,2099,1),sep=''))],1,mean))
