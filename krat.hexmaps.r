...
if (run.distn=='y')
{
	temp <- raster("H:/SpatialData/USGS GAP national/Dip_ord_OKRAx_Model/Dip_ord_OKRAx")
	temp2 <- aggregate(temp, fact=30, fun=sum, expand=FALSE, na.rm=TRUE, filename="H:/SpatialData/USGS GAP national/Dip_ord_OKRAx_Model/Dip_ord_900")
	...
	nc.2.hxn(
		...
		# buffer=500, fun=sum,
		max.value=Inf, 
		hexsim.wksp=hexsim.wksp, hexsim.wksp2=hexsim.wksp2, output.wksp=output.wksp, output.wksp2=output.wksp2, spp.folder=spp.folder, hexmap.name='krat',
