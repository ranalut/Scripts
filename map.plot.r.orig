...
map.plot <- function(the.data, distribution.data, cutoffs, spatial.data, political, ocean, model.name, add.legend, include.axes, color.ramp, margins, label.text=NA, study.area, data.type)
{
	...
	the.map@data$density[the.map@data$density < cutoffs[1]] <- cutoffs[1] + 0.001
	the.map@data$density[the.map@data$density > cutoffs[length(cutoffs)]] <- cutoffs[length(cutoffs)] - 0.001

	cutoffs.log <- sapply(cutoffs, log.transform.neg)
	the.map@data$density.log <- sapply(the.map@data$density, log.transform.neg)
	
	the.map@data$color <- as.character(cut(the.map@data$density.log, breaks=cutoffs.log, labels=color.ramp))
	...
	if (the.range2[1]<0 & data.type=='abundance') { plot(the.map[is.na(the.map@data$variable)==TRUE & the.map@data$hist.range>0,],border='white',add=TRUE) }
	...
	{ 
		...
		image.plot(legend.only=TRUE, add=TRUE, zlim=c(min(cutoffs.log),max(cutoffs.log)), breaks=cutoffs.log, col=color.ramp, lab.breaks=cutoffs, legend.width=3, graphics.reset=TRUE, legend.args=list(text=label.text,cex=1.5,side=1,line=2.25))
