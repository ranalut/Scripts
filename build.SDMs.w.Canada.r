...
data.folder <- 'nutcracker_v1' 
spp.folder <- 'woodpecker_v1' # 'woodpecker_v1' # 'nutcracker_v1' 
spp.file <- 'woodpecker' # 'woodpecker' # 'nutcracker'
threshold <- 27 # 27 # 250 # Minimum area requirement
test.train <- 	'y'; ver <- 5 # Don't forget to change the version if you turn this on.
export.hexmaps <- 'n'
build.model <- 	'y'

# map.names <- c(spp.file,'mask.water','ave.def.ann.ccsm3','ave.fire.ccsm3','ave.mtco.ccsm3', 'ave.mtwa.ccsm3','biomes.a2.ccsm3','canada')
map.names <- c(spp.file,'mask.water','ave.def.ann.ccsm3','ave.fire.ccsm3','ave.mtco.ccsm3','ave.mtwa.ccsm3','biomes.a2.ccsm3','canada2')
time.steps <- c(rep(1,6),30,1)
...
colnames(the.data) <- c('hexid',spp.file,'water','def.ann','fire','mtco','mtwa','biomes','canada')
the.data <- the.data[the.data$water!=1 & the.data$mtwa!=0 & the.data$biomes!=0,]
the.data <- the.data[-1,]
...
the.data.pred <- the.data
the.data <- the.data[the.data$canada!=1,]
# temp <- the.data; temp$biomes <- as.numeric(temp$biomes)-1;the.data <- the.data[,-2]; write.csv(temp,paste(output.wksp,'workspaces/',spp.folder,'/Analysis/the.data.', ver,'.csv',sep=''),row.names=FALSE); stop('cbw')
...
if (build.model=='y')
{
	rf.model <- randomForest(data=the.data[the.data$hexid%in%train.pts,], obs ~ def.ann + fire + mtco + mtwa + biomes, importance=TRUE)
	...
	rf.model <- randomForest(data=the.data[the.data$hexid%in%train.pts,], obs ~ def.ann + mtco + mtwa, importance=TRUE)
	# print(rf.model); print(rf.model$importance)
	save(rf.model,file=paste(output.wksp,'workspaces/',spp.folder,'/analysis/rf.model.v',ver,'b.rdata',sep=''))
	
	rf.model <- randomForest(data=the.data[the.data$hexid%in%train.pts,], obs ~ fire + biomes, importance=TRUE)
	# print(rf.model); print(rf.model$importance)
...
for (i in letters[1:3])
{
	...
	# Predict
	pred.spp.distn <- predict(rf.model, newdata=the.data.pred, type='prob')

	write.csv(data.frame(hexid=the.data.pred$hexid,Pred=pred.spp.distn[,2]), paste(output.wksp,'workspaces/',spp.folder,'/analysis/rf.model.pred.v',ver,i,'.csv',sep=''),row.names=FALSE)
}
sink()
...
write.csv(data.frame(hexid=the.data.pred$hexid,Pred=pred.spp.distn[,2]), paste(output.wksp,'workspaces/',spp.folder,'/analysis/rf.model.pred.v',ver,'.csv',sep=''),row.names=FALSE)
stop('cbw')
