...
map.plot <- function(the.data, distribution.data, cutoffs, spatial.data, political, ocean, model.name, add.legend, include.axes, color.ramp, margins, label.text=NA, study.area, data.type)
{
	...
	the.map@data$density <- the.map@data$variable/(the.map@data$F_AREA/500)
	the.range2 <- range(the.map@data$density); print(the.range2); print(unique(the.map@data$density)); dev.off(); stop('cbw')
	the.map@data$density[the.map@data$density < cutoffs[1]] <- cutoffs[1] + 1
	the.map@data$density[the.map@data$density > cutoffs[length(cutoffs)]] <- cutoffs[length(cutoffs)] - 1
	...
	if (the.range1[1]<0 & data.type=='abundance') { plot(the.map[is.na(the.map@data$density)==TRUE & the.map@data$hist.range>0,],border='white',add=TRUE) }
