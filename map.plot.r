...
map.plot <- function(the.data, distribution.data, cutoffs, spatial.data, political, ocean, model.name, add.legend, include.axes, color.ramp, margins, label.text=NA, study.area, data.type)
{
	...
	if (data.type=='abundance')
	{
		...
		# print(the.map@data[,c('variable','hist.range')])
	}
	
	# the.map@data$density <- the.map@data$variable/(the.map@data$F_AREA/100)
	# the.range2 <- range(the.map@data$density); print(the.range2)
	the.map@data$variable[the.map@data$variable < cutoffs[1]] <- cutoffs[1] + 1
	the.map@data$variable[the.map@data$variable > cutoffs[length(cutoffs)]] <- cutoffs[length(cutoffs)] - 1
	...
	if (
		dim(the.map@data[is.na(the.map@data$variable)==TRUE & the.map@data$hist.range>0,])[1] > 0 &
		the.range1[1]<0 & 
		data.type=='abundance'
		) 
		{ 
			plot(the.map[is.na(the.map@data$variable)==TRUE & the.map@data$hist.range>0,],border='white',add=TRUE)
		}
