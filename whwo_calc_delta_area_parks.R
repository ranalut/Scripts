...
path <- c('white_headed_woodpecker/white_headed_woodpecker_pa_') 
dens_thresh <- c(0.01)
...
p <- ggplot() + 
  geom_bar(data=bar_table4, aes(x=year, y=p_delta), stat='identity',width=0.35, colour="#636363", fill="#cccccc") + ylim(-80,10) + 
...
png(paste("./MS_woodpecker/Results/",species,"_wa_parks_area.png",sep=''),height=500)
  plot(p)
dev.off()

write.csv(bar_table3,paste("./MS_woodpecker/Results/",species,"_wa_parks_delta_area3.csv",sep=''))
write.csv(bar_table4,paste("./MS_woodpecker/Results/",species,"_wa_parks_delta_area4.csv",sep=''))
