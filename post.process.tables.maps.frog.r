...
post.processing.maps.frog <- function(workspace,scenario,models2,theGCMs,reps,spatial.name,ref.field,ref.values,out.name)
{
	gcm.names <- c('baseline','ccsm3','cgcm3','giss','miroc','hadcm3')
	...
	for (i in 1:length(theGCMs))
	{
		for (j in 1:length(models2))
		{
			...
			# print(head(temp))
			rownames(temp) <- paste('Y',temp$year + 2000,sep='')
			...
			if (theGCMs[i]!='baseline')
			{
				temp$d2020 <- temp$Y2020 - temp$Y2001
				temp$d2050 <- temp$Y2050 - temp$Y2001
				temp$d2100 <- temp$Y2100 - temp$Y2001
			}
