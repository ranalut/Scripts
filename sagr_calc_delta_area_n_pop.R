...
species <- 'sage-grouse'
scenario <- '_sagr_136_hab_v2b'
sens <- list(c('full','veg','clim'))
path <- c('greater_sage_grouse/greater_sage_grouse_huc_') 
dens_thresh <- c(0.01)
...
output3 <- output2
output3[,c('Y2000s','Y2020s','Y2050s','Y2090s')] <- 10 * output3[,c('Y2000s','Y2020s','Y2050s','Y2090s')] / matrix(output3[,'AREA_SQKM_'],ncol=3,nrow=dim(output3)[1])
...
p <- ggplot() + 
  geom_bar(data=bar_table4, aes(x=year, y=p_delta), stat='identity',width=0.35, colour="#636363", fill="#cccccc") + # ylim(-1,0.3) + 
...
png("D:/Box Sync/PNWCCVA/MS_SAGR/Results/pd_acreages3.png",height=500)
  plot(p)
dev.off()
write.csv(bar_table3,"D:/Box Sync/PNWCCVA/MS_SAGR/Results/delta_acreages3.csv")
# stop('cbw')
...
png("D:/Box Sync/PNWCCVA/MS_SAGR/Results/pd_population3.png",height=500)
  plot(p)
dev.off()

write.csv(bar_table3,"D:/Box Sync/PNWCCVA/MS_SAGR/Results/delta_population3.csv")
